...
| Employee directory | `GET /api/employees/directory` — **Client + Contract required** (`client` + `contractId`). Optional extra filter: one of `clientBu`, `location`, `dept`, `designation`. `q` searches inside that contract. Slim DTO includes `lastWorkingDay`. `active=yes` is derived (flag Yes **and** LWD empty or ≥ today). `active=no` is flag No **or** LWD already passed. `GET /api/employees/directory/facets?client=&contractId=` returns distinct BUs / locations / departments / designations. `GET /api/employees/directory/:id` returns one full record. Payroll Sheet loads only after Client is chosen. |
