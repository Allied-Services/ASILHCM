...
| Absence deduction (invoice) | Per manpower service: `(roleRate / 30) × absentDays` when the role has its own monthly rate; otherwise leftover line rate is split across unpriced roles. Named absences stay on `attendance_ledger`. **Unfilled SO slots** (and leavers whose last working day is before the month) write `so_deductions` `source=vacancy` as “Missing service” at the full monthly role rate — invoice compute always refreshes these. A person who worked 15 of 30 days is named at 15 × daily role rate. **Lock** still rebuilds named `attendance_ledger` shortages from locked Sheet `paid_days` (`absent = max(0, 30 − paid_days)`). |
