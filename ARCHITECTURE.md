...
| Monthly Cycle hub | Staff tab `monthly_cycle` → `MonthlyCycleHub.jsx`: Setup is **attendance + claims types only** (collection/file derived from those ticks; Attendance-only = `absent_days` file). Commercial type, Service Orders, EOBI, focals, and routing live on **Client Information → Contract**. People + Contacts seed, Collect (machine/client file upload→edit→submit **and** portal invites), **Review Desk**, Track, Corrections. Payroll push, close pack, and statutory files live on **Payroll Sheet**. Cost-plus invoice raise lives on **Invoices (AR)**. **Wafi** OT/Expense/Medical stays monthly-form. Pack still on `contract_claim_policies`. Rulebook on `contract_policies` (`commercial_type`, inferred engine, Focal, Dedicated Payroll Resource, `routing_mode` a–g, **`eobi_min_wage`** default Federal/lowest 40,000). `GET /api/fixed-value/contracts/:id/roster-capacity` and employee write enforce SO role headcount. `machine_file` writes `monthly_attendance_overrides` after submit and, for Fixed Value / Conservancy, rebuilds `so_deductions`. Email Claims / Wafi Claims / Claims Queue / **Contract Policies** / **Payroll Run** are hidden. |
