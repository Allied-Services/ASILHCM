...
| Absence deduction (invoice) | Per manpower service: `(roleRate / 30) × absentDays` when the role has its own monthly rate (Morgah seed: FM Supervisor **60,246**, Gardener **52,183**; Sihala Sweeping / Cleaning **52,043** — filled onto live lines when the stored JSON has no rate). Otherwise leftover line rate is split across unpriced roles. Named absences stay on `attendance_ledger`. **Unfilled SO slots** (and leavers whose last working day is before the month) write `so_deductions` `source=vacancy` as “Missing service” at the full monthly role rate. Invoice compute drops leftover named shortages for people who had already left, then refreshes vacancies. A person who worked 15 of 30 days is named at 15 × daily role rate. **Lock** still rebuilds named `attendance_ledger` shortages from locked Sheet `paid_days` (`absent = max(0, 30 − paid_days)`), skipping anyone not employed in the month. |
...
| Monthly Cycle hub | Staff tab `monthly_cycle` → `MonthlyCycleHub.jsx`: **collect attendance and claims only** — Setup, People, Collect, Review, Track, Corrections. No Payroll or Close tabs. Track/Review do not push or lock payroll. **Payroll Sheet** hosts Review and push + Month close. **Invoices (AR)** hosts cost-plus raise. Commercial type, Service Orders, EOBI, focals, and routing live on **Client Information → Contract**. SO chapter shows each nested service with count, rate, and a manpower flag. Pack still on `contract_claim_policies`. `machine_file` writes `monthly_attendance_overrides` after submit and, for Fixed Value / Conservancy, rebuilds `so_deductions`. Repair live PSO North Zone lines with `node backend/scripts/repair_pso_north_zone_so.js`. Email Claims / Wafi Claims / Claims Queue / **Contract Policies** / **Payroll Run** are hidden. |
