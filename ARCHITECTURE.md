...
| Contract / SO line edit roles | **Client Information → Contracts** is the master (identity, terms, sites, baseline SO lines, cost-plus rate cards, OT caps). The edit overlay is chaptered: `details` · `costs` · `so` (Fixed Value only) · `rates`. Deep link `/?tab=client&client=&contract=&chapter=so` opens Service Orders first. Monthly Cycle Setup owns the claims pack / rulebook. Fixed Value / PSO is **this month** only (attendance → billable → payroll → invoice). Contract Policies is hidden from the default nav (deep link `?tab=contract_ops` remains). Seed re-sync stays superadmin-only on the FV month tab. |
...
| Engine flag | `payroll_engine=legacy` (default): Sheet Calculate/lock/AP pay. `runs`: Sheet write/calculate `409 CONTRACT_ON_RUNS_ENGINE`; compute without flip `409 CONTRACT_ON_LEGACY_ENGINE`. **Fixed Value / Conservancy** (`commercial_type=fixed_value`, SO billing, or `CTR-PSO-*`) may compute a run without a stored `runs` flip — that hub has no sheet path. Do not flip production cost-plus contracts without `Go red:`. |
