...
| Attendance ingest | Excel sheet `"{MonthName} {year}"` or Google Drive folder `DRIVE_ATTENDANCE_FOLDER_ID`; override stores `present_days` + `absent_days`. **Monthly Cycle Collect submit** also writes those overrides (`source=cycle_machine_file`, including `absent_days`) and refreshes `so_deductions` (`attendance_ledger`) for every submitted employee on an FV contract — no Compute ALL required. |
| Single write path | `backend/src/modules/serviceOrders/contractCrud.js` (wizard, CORO seed, NZ re-sync) |
| Contract / SO line edit roles | **Client Information → Contracts** is the master (identity, terms, sites, baseline SO lines, cost-plus rate cards, OT caps). The edit overlay is chaptered: `details` · `costs` · `so` (Fixed Value only) · `rates`. Deep link `/?tab=client&client=&contract=&chapter=so` opens Service Orders first. Monthly Cycle Setup owns the claims pack / rulebook. **Month Invoices** raises the SO pack without a second attendance pull. Payroll for FV is scoped to `employees.contract_id` only. Contract Policies is hidden from the default nav (deep link `?tab=contract_ops` remains). Seed re-sync stays superadmin-only on `?tab=fixed_value`. |
