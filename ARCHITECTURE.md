...
| Engine flag | `payroll_engine=legacy` (default): Sheet Calculate/lock/AP pay. `runs`: Sheet write/calculate `409 CONTRACT_ON_RUNS_ENGINE`; compute without flip `409 CONTRACT_ON_LEGACY_ENGINE`. **Fixed Value / Conservancy** (`commercial_type=fixed_value` or SO billing) may compute a run without a stored `runs` flip — that hub has no sheet path. Contract id prefixes are not used to guess the engine. Do not flip production cost-plus contracts without `Go red:`. |
