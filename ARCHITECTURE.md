...
| Absence deduction (invoice) | Per nested SO service: `(role.rate / calendarDaysInMonth) × absentDays`. Sihala Sweeping / Cleaning is **52,043** per resource → August 1 day = **1,678.81**. Dedicated one-role lines (Pesh Imam, CORO) use `line.rate / count`. Kitchen-sink lines never split the lump across all headcount — missing nested rate is `missing_service_rate`. Position Keywords on each service bind roster titles (Janitor → Sweeping). Named absences stay on `attendance_ledger`. **Unfilled SO slots** write `source=vacancy` at the full monthly unit rate. **Lock** still converts Sheet `paid_days` with `absent = max(0, 30 − paid_days)` but the daily rate uses calendar days. |
...
- Rebuild FV shortage amounts from attendance: `node backend/scripts/rebuild_fv_so_deductions.js --contract CTR-PSO-NORTH-ZONE --month 8 --year 2026` (dry-run rolls back; `--apply` / `--allow-production`)
