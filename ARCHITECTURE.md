...
| Employee directory | `GET /api/employees/directory` — search-first list (`q`, `bu`, `client`, `contractId`, `clientBu`, `location`, `dept`, `active=yes\|no\|all`, `page`, `limit`≤100, `sort`, `browse=1`). Slim DTO only (no bank/family/medical/address). Requires 2+ character `q`, an org filter, or `browse=1`. Same cutover visibility as `GET /api/employees`. `GET /api/employees/directory/:id` returns one full record for the profile pane. `GET /api/employees` still returns the full roster when unfiltered (Employee Information and other screens). Payroll Sheet loads only after Client is chosen, via `GET /api/employees?client=&contractId=` and `GET /api/payroll/:year/:month?client=&contractId=&location=`. |
