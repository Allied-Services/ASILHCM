...
| Absence deduction (invoice) | Per nested SO service: `(role.rate / calendarDaysInMonth) × absentDays`. Unit rates are **site-specific** (Sihala Sweeping **52,043** → August 1 day = **1,678.81**; Faqirabad Conservancy **60,222**; Chakpirana Sweeping **52,046**). Dedicated one-role lines use `line.rate / count`. Kitchen-sink lines never leftover-split the lump. Position Keywords bind roster titles (Janitor → Sweeping). Named absences stay on `attendance_ledger`. **Unfilled SO slots** write `source=vacancy` at the full monthly unit rate. **Lock** still converts Sheet `paid_days` with `absent = max(0, 30 − paid_days)` but the daily rate uses calendar days. |
