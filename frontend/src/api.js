...
export async function apiFetch(path, options = {}) {
    ...
    if (!res.ok) {
        ...
        throw new Error((err.error || `Request failed: ${res.status}`) + detail);
    }
    return res.json();
}
...
export const api = {
    ...
    getPayrollBankReadiness: (year, month, q = {}) => {
        const params = new URLSearchParams();
        Object.entries(q || {}).forEach(([k, v]) => {
            if (v == null || v === '' || v === 'All') return;
            params.set(k, String(v));
        ...
        const qs = params.toString();
        return apiFetch(`/api/payroll/${year}/${month}/bank-readiness${qs ? `?${qs}` : ''}`);
    },
    ...
    portalClaimsCampaignFilters: () => apiFetch('/api/portal-claims/campaign/filters'),
    portalClaimsFilters: () => apiFetch('/api/portal-claims/admin/filters'),
  ...
  // ── Xero bill import / billable invoicing ────────────────────────────────────
    ...
    updateFixedValueInvoiceNumber: (invoiceId, invoiceNumber) => apiFetch(
        `/api/fixed-value/invoices/${encodeURIComponent(invoiceId)}/invoice-number`,
        { method: 'PATCH', body: JSON.stringify({ invoice_number: invoiceNumber }) }
    ),
    ...
    sendFixedValueVerificationEmails: (contractId, month, year, payload = {}) => apiFetch(
        ...
        { method: 'POST', body: JSON.stringify({ month, year, ...payload }) }
    ),
    ...
    listOrgContacts: (q = {}) => apiFetch('/api/records/contacts?' + new URLSearchParams(q).toString()),
    upsertOrgContact: (d) => apiFetch('/api/records/contacts', { method: 'POST', body: JSON.stringify(d) }),
    seedOrgContacts: () => apiFetch('/api/records/contacts/seed', { method: 'POST', body: '{}' }),
    listRegions: () => apiFetch('/api/records/regions'),
    upsertRegion: (d) => apiFetch('/api/records/regions', { method: 'PUT', body: JSON.stringify(d) }),
    ...
    submitCycleFile: (id) => apiFetch(`/api/cycle-files/${id}/submit`, { method: 'POST', body: '{}' }),
    getPayrollProvenance: (year, month, q = {}) => apiFetch(`/api/payroll/${year}/${month}/provenance?` + new URLSearchParams(q).toString()),
    detectPayrollConflicts: (year, month) => apiFetch(`/api/payroll/${year}/${month}/detect-conflicts`, { method: 'POST', body: '{}' }),
    resolvePayrollConflict: (id) => apiFetch(`/api/payroll/conflicts/${id}/resolve`, { method: 'POST', body: '{}' }),
    importPendingClaims: (year, month, employeeIds) => apiFetch(`/api/payroll/${year}/${month}/import-pending`, { method: 'POST', body: JSON.stringify({ employeeIds }) }),
    getPayrollEngines: (contractIds) => apiFetch('/api/records/engines?contractIds=' + encodeURIComponent((contractIds || []).join(','))),
    getMonthClose: (contractId, year, month) => apiFetch(`/api/month-close/${encodeURIComponent(contractId)}/${year}/${month}`),
    raiseCostPlusInvoice: (contractId, year, month) => apiFetch(`/api/month-close/${encodeURIComponent(contractId)}/${year}/${month}/invoice`, { method: 'POST', body: '{}' }),
    createSheetClosePack: (contractId, year, month) => apiFetch(`/api/month-close/${encodeURIComponent(contractId)}/${year}/${month}/close-pack`, { method: 'POST', body: '{}' }),
    getStatutoryFiles: (year, month, contractId) => apiFetch(`/api/payroll/${year}/${month}/statutory-files` + (contractId ? `?contractId=${encodeURIComponent(contractId)}` : '')),
};
