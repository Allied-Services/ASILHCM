...
PSO Service Order lines keep role monthly rates in `roles` JSONB. Absence shortages use the matched role rate ÷ 30 × days (equal split only if a role has no rate). Unfilled role slots are billed by `vacancySync` (Morgah FM Supervisor 60,246; one Gardener 52,183). Clients → Service Orders shows manpower / non-manpower site totals and warns when priced services do not match the line total. Monthly Cycle stays collection-only. Payroll Sheet hosts Review and push + Month close; Invoices (AR) hosts cost-plus raise. Repair live SO lines with `node backend/scripts/repair_pso_north_zone_so.js` (dry-run; `--apply` / `--allow-production`). Team how-to: `docs/MONTHLY_CYCLE_SEP2026_TEAM_HOWTO.md`.
...
### 2026-09-05 — Directory default All; Inactive no longer empty
Employee Information status defaults to **All**. Inactive used the cutover “must be Active” clause and returned nobody (e.g. `ASIL/PSO-180/25` Dabeer Ahmad). Active/Inactive now filter `employees.active` only; a name/code search also skips the pre-Jul-2026 last-working-day hide.
...
### 2026-08-28 — Payroll Sheet honors User Management edit rights
Cell save (`POST /api/payroll/:year/:month`) was `requireRole('finance_proposer')` only, so a user granted Payroll Sheet in User Management (e.g. Sadia) could open the tab but got **Forbidden: insufficient role** on every value change. Save and Calculate now allow payroll roles **or** `hcm_users.permissions.payroll` with `edit` (same pattern as Portal Claims custom perms). Lock/unlock unchanged.
