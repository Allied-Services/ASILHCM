...
function findLineForDesignation(lines, designation) {
    ...
    for (const line of lines || []) {
        ...
        const roles = Array.isArray(line.roles) ? line.roles : (typeof line.roles === 'string' ? JSON.parse(line.roles || '[]') : []);
        if (roles.some(r => normalizeDesignation(r.designation) === target)) return { line, roles };
        if (roles.some(r => target.includes(normalizeDesignation(r.designation)) || normalizeDesignation(r.designation).includes(target))) {
            return { line, roles };
        }
    }
    return null;
