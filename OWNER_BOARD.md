...
> Last updated: 2026-09-22 · Keep under ~100 lines. Plain English only.
...
STATUS: **AMBER** — Live site OK (health commit `63f01bc`, 22 Sep). Live email and SMS are **on** (Go red 3 Sep). August payslip resend still open after the blocked send. Do not Calculate August. Morning brief Automation runs weekdays but **cannot email yet** (no send tool).
...
**Live site OK** (`63f01bc`). **Live email and SMS are on.** August payslip resend still needs owner confirm. Do not Calculate August. Morning email delivery still disconnected.
...
11. **Morning brief Automation** — weekday cron is on; email delivery still not connected (no send tool on the Automation)
...
- **PR #206** — Persist EOBI/SESSI + cost-plus invoice stamp; **merge conflicts** — fix or close
- **Connect morning email** on the Automation so weekday briefs actually arrive
