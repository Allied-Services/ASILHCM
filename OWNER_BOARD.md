...
> Last updated: 2026-09-17 · Keep under ~100 lines. Plain English only.
...
STATUS: **AMBER** — Live site OK (commit `6d23b89`). Live email and SMS are **on**. August payslip resend still open. Do not Calculate August. Morning email still not connected on the Automation.
...
11. **Morning brief Automation** — weekday runs write the brief + PR; **email send still not connected** (owner must enable email on the Automation)
...
- Confirm **August payslip resend** finished (or `Go red:` if more live send needed)
- Connect **morning email** on the Automation so weekday briefs reach your inbox
