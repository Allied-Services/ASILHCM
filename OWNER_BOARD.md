...
> Last updated: 2026-09-15 · Keep under ~100 lines. Plain English only.
...
STATUS: **AMBER** — Live email and SMS are **on** (Go red 3 Sep). August payslip resend still open. Do not Calculate August. Live health 15 Sep: OK (commit `1eba057`).
...
11. **Morning brief Automation** — weekday runs on; email delivery still not connected (brief lands in PR + HTML card only)
...
- Confirm **August payslip resend** is done (or say when to run it) — 304 people after live mail switch-on
- Connect **morning email** delivery on the Automation (digests stay email-only)
