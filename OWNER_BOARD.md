...
STATUS: **AMBER** — Live email and SMS are **on** (Go red 3 Sep). August payslip resend still open. Do not Calculate August. Live health OK (`ad40abec`). Morning email delivery still not connected.
...
**Live email and SMS are on.** August payslip resend still open after the blocked send (0/304). Do not Calculate August. Open work PR needing owner: **#206**.
...
11. **Morning brief Automation** — weekday cron runs; email delivery still not connected (HTML card + PR only)
...
## BLOCKED ON YOU
- Review / decide **PR #206** (EOBI/SESSI persist + cost-plus invoice stamp)
- Confirm **August payslip resend** finished (or still stuck)
- Connect **morning brief Automation email** (weekday runs still cannot send Subject `ASIL HCM — morning brief`)
